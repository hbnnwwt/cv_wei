...
\begin{frame}{本周分组策略}
    \begin{columns}
        ...
        \begin{itemize}
            \item 每4人为一组
            \item 确保不同专业背景混合
            \item 含：理工科、文科、有/无编程基础
        \end{itemize}

        \column{0.5\textwidth}
        \textbf{角色分工：}
        \begin{itemize}
            \item \textbf{组长}：统筹协调、进度管理
            \item \textbf{算法实现者}：边缘检测、轮廓检测
            \item \textbf{参数调优者}：Canny阈值、筛选参数
            \item \textbf{测试者}：收集用例、报告问题
        \end{itemize}
    \end{columns}
...
\begin{frame}{并行学习路径（二）}
    ...
    \begin{alertblock}{选择建议}
        根据个人基础选择路径，可逐步进阶：观察者 $\rightarrow$ 使用者 $\rightarrow$ 创造者
    \end{alertblock}
\end{frame}
...
\begin{frame}{课堂互动环节设计}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item \textbf{内容}：将Canny边缘检测代码打乱成5块
            \item \textbf{任务}：小组竞赛复原代码，正确运行
    ...
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item \textbf{问题}：findContours 输入彩色图而非灰度图
            \item \textbf{目的}：培养调试能力，预防常见错误
        \end{itemize}

        \column{0.5\textwidth}
        \textbf{互动4：参数调试挑战（限时5分钟）}
        \begin{itemize}
            \item \textbf{时机}：Canny 参数调优页面讲解后
            \item \textbf{任务}：调整低/高阈值，让边缘检测"刚好"
            \item \textbf{评判}：检测到的边缘既不多也不少
            \item \textbf{目的}：培养参数敏感度和工程直觉
        \end{itemize}
    \end{columns}
\end{frame}
