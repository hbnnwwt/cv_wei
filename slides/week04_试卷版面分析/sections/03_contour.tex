...
\begin{frame}[fragile]{课堂互动：错误找茬}
    \begin{block}{题目（集体讨论，3分钟）}
        以下代码运行时出错，请找出错误原因并修正。
    \end{block}
...
contours, _ = cv2.findContours(
    ...
    cv2.CHAIN_APPROX_SIMPLE)
        ...
        \begin{alertblock}{Bug}
            \texttt{Canny(img, ...)} 用了彩色图！
        \end{alertblock}

        \begin{block}{修正}
            \texttt{gray = cv2.cvtColor(img,} \\
            \texttt{~~~~~ cv2.COLOR\_BGR2GRAY)} \\
            \texttt{edges = cv2.Canny(gray, ...)}
        \end{block}
    \end{columns}
\end{frame}
