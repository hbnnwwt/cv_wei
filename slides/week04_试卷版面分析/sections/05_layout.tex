...
def find_divider_lines(proj, threshold=10, min_length=5):
    """
    Find divider lines (valleys)
    proj: projection array
    threshold: valley threshold
    min_length: min consecutive length (noise filter)
    """
    dividers = []
    current_start = None
...
def layout_analysis(binary):
    """
    Layout analysis: find question dividers
    """
    # horizontal projection
    h_proj = np.sum(binary // 255, axis=1)

    # find divider lines
    threshold = np.mean(h_proj) * 0.1  # adaptive threshold
    dividers = find_divider_lines(h_proj, threshold)

    # compute question regions
    regions = []
    prev_end = 0
...
# iterate all components (skip bg label=0)
for i in range(1, num_labels):
    x, y, w, h, area = stats[i]
    cx, cy = centroids[i]

    # filter: area within range
    if 100 < area < 10000:
