...
\begin{frame}{RTF 框架实战对比}
	...
	\begin{columns}
		\column{0.5\textwidth}
		\begin{alertblock}{\textcolor{red}{\textbf{[X]}} 不好的 Prompt}
			帮我写个代码处理图像。
		\end{alertblock}
		...
		\column{0.5\textwidth}
		\begin{exampleblock}{\textcolor{green}{\textbf{[V]}} 好的 Prompt (RTF)}
...
\begin{frame}{进阶技巧：Chain-of-Thought (思维链)}
	...
	\begin{columns}
		\column{0.5\textwidth}
		\begin{alertblock}{\textcolor{red}{\textbf{[X]}} 直接给答案}
		...
		\column{0.5\textwidth}
		\begin{exampleblock}{\textcolor{green}{\textbf{[V]}} 思维链引导}
			\textbf{Prompt：}
...
\begin{frame}{Few-shot 效果对比}
	\begin{table}
		...
		\begin{tabular}{p{2.5cm}p{3.5cm}p{3.5cm}}
			\toprule
			\textbf{Prompt} & \textbf{无 Few-shot} & \textbf{有 Few-shot} \\
			\midrule
			代码风格 & 随机，不稳定 & 与示例高度一致 \\
			注释完整度 & 较简略 & 详细规范 \\
			参数说明 & 缺失 & 完整 Docstring \\
			错误处理 & 经常遗漏 & 按示例添加 \\
			代码可读性 & 一般 & 优秀 \\
			\bottomrule
		\end{tabular}
...
\begin{frame}{上下文窗口管理}
	\tiny
	\begin{alertblock}{问题：AI为什么会"忘"掉之前的内容？}
		上下文窗口有限，超出后最早的内容会被"遗忘"
	\end{alertblock}
...
\begin{frame}{精简Prompt技巧}
	\tiny
	\vspace{-0.8cm}
	\begin{enumerate}
		\item \textbf{只提供必要代码}
		\item \textbf{使用摘要}："前面讨论了X，现在解决Y"
		\item \textbf{分段处理}：长任务拆分成多个短任务
		\item \textbf{定期总结}：让AI总结当前进度
	\end{enumerate}
\end{frame}
