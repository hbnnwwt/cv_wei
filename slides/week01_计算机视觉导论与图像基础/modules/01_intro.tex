...
\begin{frame}{选择适合你的学习路径}
    ...
    \begin{center}
        \textbf{你可以随时在不同路径间切换！}\\
        \small 本课程鼓励互助学习，创造者路径的同学可以协助观察者路径的同学
    \end{center}
\end{frame}
...
\begin{frame}[fragile]{视觉：人类获取信息的最主要渠道}
    \begin{columns}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{images/semantic_vs_matrix.png}
            \caption{人类视觉（语义） vs 计算机视觉（数字矩阵）}
        \end{figure}
    \end{columns}
\end{frame}
...
\begin{frame}[fragile]{CV 的历史与现状}
    \begin{columns}
        ...
        \begin{itemize}
            ...
            \item \textbf{1999}：SIFT特征提取算法的诞生
            \item \textbf{2012}：\highlight{深度学习爆发}，AlexNet 在 ImageNet 竞赛中夺冠。
        \end{itemize}
        ...
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{images/cv_history_timeline.png}
        \end{figure}
    \end{columns}
\end{frame}

\begin{frame}[fragile]{深度学习的突破：ImageNet 竞赛}
    \begin{columns}
        \column{0.45\textwidth}
        \begin{block}{传统方法 (2011)}
            手工设计特征 \\
            SIFT / HOG + SVM/随机森林
            \vspace{0.3cm}
            \centering
    ...
    \vspace{0.2cm}
    \begin{center}
        \LARGE \highlight{提升幅度：+12\%} \\
        \textit{这是计算机视觉领域的"AlphaGo时刻"！}
    \end{center}

    \vspace{0.3cm}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\textwidth]{images/alexnet_comparison.png}
    \end{figure}
\end{frame}

\begin{frame}[fragile]{ImageNet 竞赛：准确率飙升}
    \begin{columns}
        \column{0.55\textwidth}
        \begin{figure}
            \centering
            \includegraphics[width=\textwidth]{images/imagenet_trend.png}
        \end{figure}

        \column{0.45\textwidth}
        \begin{block}{关键启示}
            \begin{itemize}
                \item \textbf{2012年前}：传统方法停滞
                \item \textbf{2012年后}：深度学习爆发
                \item \textbf{现在}：超越人类
            \end{itemize}
        \end{block}
    \end{columns}
...
% -----------------------------------------------------------------------------
% 答题卡定位锚点（Timing Marks）—— 进阶内容
% -----------------------------------------------------------------------------

\begin{frame}[fragile]{🎓 进阶：答题卡的"定位锚点" (Timing Marks)}
...
\begin{frame}[fragile]{🎓 进阶：定位锚点检测——从轮廓到坐标}
