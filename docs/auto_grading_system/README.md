...
| 编号 | 问题 | 文件 | 修复内容 |
|------|------|------|----------|
| H90-H91 | `save_result_xlsx` 内用字面量 `3`/`2` 而非实例变量 | `grading.py:201,206` | 使用 `self.choice_score`/`self.judge_score` |
| H92-H93 | UI 显示 "20分" 硬编码 | `app.py:755,1011` | 改为 `svc.essay_max_score` 动态读取 |
| H127 | pipeline 默认阈值 0.5 与识别器 0.06 不一致 | `pipeline.py:32,47` | 统一为 0.06，与 GUI slider 默认值一致 |
